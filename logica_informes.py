...
def ejecutar_fase_1(load_ids, fecha_cierre_dt, plantillas_bytes, credenciales, drive_service):
    """
    ...
    Informe_VE_01= f"""  SELECT
                        ...
                        FROM pro_pichincha_alquid.metric
                        WHERE load_id IN (
                        ...
                        GROUP BY 1
                      -- Aquí va el SELECT principal completo con CASE y SUM por escenario
                      UNION ALL
                      ...
                      AND dim_1 in ('P_Cuentas de otras EECC', 'P_ECORP_P. Plazo', 'P_PIB_P. Plazo', 'P_POFI_P. Plazo', 'P_SSCC_P. Plazo',  'P_ECORP_Ctas No Remuneradas', 'P_EESP_Ctas No Remuneradas', 'P_PIB_Ctas No Remuneradas',
                              'P_POFI_Ctas No Remuneradas', 'P_SSCC_Ctas No Remuneradas', 'P_SSCCFRO_Ctas No Remuneradas',
    ...
    WHEN '1.3 DEPOSITOS OTRAS EECC' THEN 3
    WHEN '1.4 INVERSION CREDITICIA' THEN 4
    WHEN '1.6 ACTIVOS DUDOSOS' THEN 5
    WHEN '1.7 ACTIVOS NO SENSIBLE' THEN 6
    WHEN '2.1 DEPOSITOS OTRAS EECC' THEN 7
    WHEN '2.2 DEPOSITOS A PLAZO DE CLIENTES' THEN 8
    WHEN '2.5 CARTERA MAYORISTA' THEN 9
    WHEN '2.4 PASIVOS NO SENSIBLES' THEN 10
    WHEN '3.1 DERIVADOS' THEN 11
    WHEN '4.1 PN' THEN 12
    WHEN '1.5 RENTA FIJA' THEN 997
    WHEN '2.3 DEPOSITOS A LA VISTA DE CLIENTES' THEN 998
    ELSE 999
    END;"""
    ...
    Informe_VE_EBA_01= f""" -- SELECT principal
                            SELECT
                              CASE
                                WHEN dim_1 IN ('A_Cuentas en otras EECC', 'A_Tesoreria') THEN '1.1 BANCOS'
                                WHEN dim_1 IN ('A_Ata') THEN '1.2 CARTERA MAYORISTA'
    ...
    Informe_MF_MTO= f"""SELECT *
                        FROM (
                        -- Línea en blanco tras 1.7
                        SELECT ' ' AS Jerarquia, NULL AS Dwn200_stck, NULL AS Dwn200_Nb, NULL AS Dwn100_stck, NULL AS Dwn100_Nb,  NULL AS Dwn75_stck, NULL AS Dwn75_Nb, NULL AS Dwn50, NULL AS Dwn50_Nb, NULL AS Dwn25_stck, NULL AS Dwn25_Nb, NULL AS Base_stck, NULL AS Base_Nb, NULL AS Up25_stck, NULL AS Up25_Nb, NULL AS Up50_stck, NULL AS Up50_Nb, NULL AS Up75_stck, NULL AS Up75_Nb, NULL AS Up100_stck, NULL AS Up100_Nb, NULL AS Up200_stck, NULL AS Up200_Nb
                        UNION ALL
                        SELECT
                            CASE
                            WHEN dim_1 IN ('A_Cuentas en otras EECC', 'A_Tesoreria') THEN '1.1 BANCOS'
                        ...
                        FROM pro_pichincha_alquid."result"
                            WHERE load_id IN (
                            '{load_ids['cierre_base']}',
                            '{load_ids['cierre_up']}',
                            '{load_ids['cierre_dwn']}'
                            )
                        GROUP BY 1) AS tabla_union
                        -- Aquí va el SELECT principal completo con CASE y SUM por escenario
                        UNION ALL
                        SELECT 'TOTAL GRUPO 1' AS Jerarquia,
                            SUM(CASE WHEN scenario = 'Dwn200' AND not is_nb  THEN (incexp + diff_incexp_yield)  ELSE 0 END) AS Dwn200_stck,
    ...
    Informe_MF_EBA= f"""SELECT *
                        FROM (
                        -- Línea en blanco tras 1.7
                        SELECT '  ' AS Jerarquia, NULL AS Dwn200_stck, NULL AS Dwn200_Nb, NULL AS ShortRDwn_stck, NULL AS ShortRDwn_Nb,
                            NULL AS Steep_stck, NULL AS Steep_Nb, NULL AS Base_stck, NULL AS Base_Nb, NULL AS Flat_stck, NULL AS Flat_Nb,
                            NULL AS ShortRUP_stck, NULL AS ShortRUP_Nb, NULL AS Up200_stck, NULL AS Up200_Nb
                        UNION ALL
                        -- Línea en blanco tras 2.4
                        SELECT ' ' AS Jerarquia, NULL AS Dwn200_stck, NULL AS Dwn200_Nb, NULL AS ShortRDwn_stck, NULL AS ShortRDwn_Nb,
                            NULL AS Steep_stck, NULL AS Steep_Nb, NULL AS Base_stck, NULL AS Base_Nb, NULL AS Flat_stck, NULL AS Flat_Nb,
                            NULL AS ShortRUP_stck, NULL AS ShortRUP_Nb, NULL AS Up200_stck, NULL AS Up200_Nb
                        UNION ALL
    ...
    Informe_Detalle_VF= f"""SELECT
                            ...
                            NULLIF(SUM(CASE WHEN SCENARIO = 'Dwn200' THEN ABS(market_value) ELSE 0 END), 0)/100 AS effective_duration_PONDERADA,
                            SUM(CASE WHEN SCENARIO = 'Dwn200' THEN ABS(macaulay_duration * market_value) ELSE 0 END) /
                            NULLIF(SUM(CASE WHEN SCENARIO = 'Dwn200' THEN ABS(market_value) ELSE 0 END), 0) AS DURACION_MACAULAY_PONDERADA,
                            SUM(CASE WHEN scenario = 'Dwn200' THEN market_value ELSE 0 END) AS Dwn200,
                            SUM(CASE WHEN SCENARIO = 'Dwn100' THEN ABS(effective_duration * market_value) ELSE 0 END) /
                            NULLIF(SUM(CASE WHEN SCENARIO = 'Dwn100' THEN ABS(market_value) ELSE 0 END), 0)/100 AS effective_duration_PONDERADA,
                            SUM(CASE WHEN SCENARIO = 'Dwn100' THEN ABS(macaulay_duration * market_value) ELSE 0 END) /
                            NULLIF(SUM(CASE WHEN SCENARIO = 'Dwn100' THEN ABS(market_value) ELSE 0 END), 0) AS DURACION_MACAULAY_PONDERADA,
                            SUM(CASE WHEN scenario = 'Dwn100' THEN market_value ELSE 0 END) AS Dwn100,
                            SUM(CASE WHEN SCENARIO = 'Dwn75' THEN ABS(effective_duration * market_value) ELSE 0 END) /
                            NULLIF(SUM(CASE WHEN SCENARIO = 'Dwn75' THEN ABS(market_value) ELSE 0 END), 0)/100 AS effective_duration_PONDERADA,
                            SUM(CASE WHEN SCENARIO = 'Dwn75' THEN ABS(macaulay_duration * market_value) ELSE 0 END) /
                            NULLIF(SUM(CASE WHEN SCENARIO = 'Dwn75' THEN ABS(market_value) ELSE 0 END), 0) AS DURACION_MACAULAY_PONDERADA,
                            SUM(CASE WHEN scenario = 'Dwn75' THEN market_value ELSE 0 END) AS Dwn75,
                            SUM(CASE WHEN SCENARIO = 'Dwn50' THEN ABS(effective_duration * market_value) ELSE 0 END) /
                            NULLIF(SUM(CASE WHEN SCENARIO = 'Dwn50' THEN ABS(market_value) ELSE 0 END), 0)/100 AS effective_duration_PONDERADA,
                            SUM(CASE WHEN SCENARIO = 'Dwn50' THEN ABS(macaulay_duration * market_value) ELSE 0 END) /
                            NULLIF(SUM(CASE WHEN SCENARIO = 'Dwn50' THEN ABS(market_value) ELSE 0 END), 0) AS DURACION_MACAULAY_PONDERADA,
                            SUM(CASE WHEN scenario = 'Dwn50' THEN market_value ELSE 0 END) AS Dwn50,
                            SUM(CASE WHEN SCENARIO = 'Dwn25' THEN ABS(effective_duration * market_value) ELSE 0 END) /
                            NULLIF(SUM(CASE WHEN SCENARIO = 'Dwn25' THEN ABS(market_value) ELSE 0 END), 0)/100 AS effective_duration_PONDERADA,
                            SUM(CASE WHEN SCENARIO = 'Dwn25' THEN ABS(macaulay_duration * market_value) ELSE 0 END) /
                            NULLIF(SUM(CASE WHEN SCENARIO = 'Dwn25' THEN ABS(market_value) ELSE 0 END), 0) AS DURACION_MACAULAY_PONDERADA,
                            SUM(CASE WHEN scenario = 'Dwn25' THEN market_value ELSE 0 END) AS Dwn25,
                            SUM(CASE WHEN SCENARIO = 'Base' THEN ABS(effective_duration * market_value) ELSE 0 END) /
                            NULLIF(SUM(CASE WHEN SCENARIO = 'Base' THEN ABS(market_value) ELSE 0 END), 0)/100 AS effective_duration_PONDERADA,
                            SUM(CASE WHEN SCENARIO = 'Base' THEN ABS(macaulay_duration * market_value) ELSE 0 END) /
                            ...
                            NULLIF(SUM(CASE WHEN SCENARIO = 'Up25' THEN ABS(market_value) ELSE 0 END), 0)/100 AS effective_duration_PONDERADA,
                            SUM(CASE WHEN SCENARIO = 'Up25' THEN ABS(macaulay_duration * market_value) ELSE 0 END) /
                            NULLIF(SUM(CASE WHEN SCENARIO = 'Up25' THEN ABS(market_value) ELSE 0 END), 0) AS DURACION_MACAULAY_PONDERADA,
                            SUM(CASE WHEN scenario = 'Up25' THEN market_value ELSE 0 END) AS Up25,
                            SUM(CASE WHEN SCENARIO = 'Up50' THEN ABS(effective_duration * market_value) ELSE 0 END) /
                            NULLIF(SUM(CASE WHEN SCENARIO = 'Up50' THEN ABS(market_value) ELSE 0 END), 0)/100 AS effective_duration_PONDERADA,
                            SUM(CASE WHEN SCENARIO = 'Up50' THEN ABS(macaulay_duration * market_value) ELSE 0 END) /
                            NULLIF(SUM(CASE WHEN SCENARIO = 'Up50' THEN ABS(market_value) ELSE 0 END), 0) AS DURACION_MACAULAY_PONDERADA,
                            SUM(CASE WHEN scenario = 'Up50' THEN market_value ELSE 0 END) AS Up50,
                            SUM(CASE WHEN SCENARIO = 'Up75' THEN ABS(effective_duration * market_value) ELSE 0 END) /
                            NULLIF(SUM(CASE WHEN SCENARIO = 'Up75' THEN ABS(market_value) ELSE 0 END), 0)/100 AS effective_duration_PONDERADA,
                            SUM(CASE WHEN SCENARIO = 'Up75' THEN ABS(macaulay_duration * market_value) ELSE 0 END) /
                            NULLIF(SUM(CASE WHEN SCENARIO = 'Up75' THEN ABS(market_value) ELSE 0 END), 0) AS DURACION_MACAULAY_PONDERADA,
                            SUM(CASE WHEN scenario = 'Up75' THEN market_value ELSE 0 END) AS Up75,
                            SUM(CASE WHEN SCENARIO = 'Up100' THEN ABS(effective_duration * market_value) ELSE 0 END) /
                            NULLIF(SUM(CASE WHEN SCENARIO = 'Up100' THEN ABS(market_value) ELSE 0 END), 0)/100 AS effective_duration_PONDERADA,
                            SUM(CASE WHEN SCENARIO = 'Up100' THEN ABS(macaulay_duration * market_value) ELSE 0 END) /
                            NULLIF(SUM(CASE WHEN SCENARIO = 'Up100' THEN ABS(market_value) ELSE 0 END), 0) AS DURACION_MACAULAY_PONDERADA,
                            SUM(CASE WHEN scenario = 'Up100' THEN market_value ELSE 0 END) AS Up100,
                            SUM(CASE WHEN SCENARIO = 'Up200' THEN ABS(effective_duration * market_value) ELSE 0 END) /
                            NULLIF(SUM(CASE WHEN SCENARIO = 'Up200' THEN ABS(market_value) ELSE 0 END), 0)/100 AS effective_duration_PONDERADA,
                            SUM(CASE WHEN SCENARIO = 'Up200' THEN ABS(macaulay_duration * market_value) ELSE 0 END) /
                            NULLIF(SUM(CASE WHEN SCENARIO = 'Up200' THEN ABS(market_value) ELSE 0 END), 0) AS DURACION_MACAULAY_PONDERADA,
                            SUM(CASE WHEN scenario = 'Up200' THEN market_value ELSE 0 END) AS Up200
                            FROM pro_pichincha_alquid.metric
                            WHERE load_id IN (
                            '{load_ids['cierre_base']}',
                            ...
                            WHEN '1.6 ACTIVOS DUDOSOS' THEN 6
                            WHEN '1.7 ACTIVOS NO SENSIBLE' THEN 7
                            WHEN '2.1 DEPOSITOS OTRAS EECC' THEN 8
                            WHEN '2.2 DEPOSITOS A PLAZO DE CLIENTES' THEN 9
                            WHEN '2.3 DEPOSITOS A LA VISTA DE CLIENTES' THEN 10
                            WHEN '2.4 PASIVOS NO SENSIBLES' THEN 11
                            WHEN '2.5 CARTERA MAYORISTA' THEN 12
                            WHEN '3.1 DERIVADOS' THEN 13
                            WHEN '4.1 PN' THEN 14
                            ELSE 999
                            END; """
    ...
    Informe_Detalle_MF= f"""SELECT *
    FROM (
        SELECT ' ' AS JER_PRODUCTO,
            NULL AS Dwn200_stck,
            NULL AS Dwn200_Nb,
            NULL AS Base_stck,
            NULL AS Base_Nb
        ...
        SELECT '  ' AS JER_PRODUCTO,
            ...
            NULL AS Base_Nb
        UNION ALL
        -- Añadir explícitamente las filas "ACT DUDOSOS" y "A_ECORP_Otros_Tarjetas Credito_Gtia Personal"
        -- Esto asegura que siempre aparezcan, incluso si no hay datos de origen.
        SELECT 'A_ECORP_Otros_Tarjetas Credito_Gtia Personal' AS JER_PRODUCTO,
            NULL AS Dwn200_stck,
            NULL AS Dwn200_Nb,
            NULL AS Base_stck,
            NULL AS Base_Nb
        ...
        SELECT
            CASE
                ...
                WHEN dim_1 = 'A_Balancer' THEN 'A_Balancer' -- Mantener la categoría, pero su valor será 0
                WHEN dim_1 LIKE '%Activo Corto Plazo%' THEN 'ACTIVO CORTO PLAZO'
                WHEN dim_1 LIKE '%Ant. Nomina%' THEN 'ANTICIPO NOMINA'
                -- Modificación para CREDITOS: excluye los dudosos
                WHEN dim_1 LIKE '%_Cred.%' AND dim_1 NOT IN ('A_Cred._Dudoso') THEN 'CREDITOS'
                WHEN dim_1 LIKE '%Descubiertos%' THEN 'DESCUBIERTOS'
                -- Modificación para PRESTAMOS: excluye los dudosos
                WHEN dim_1 LIKE '%_Prest.%' AND dim_1 NOT IN ('A_Prest._Dudoso') THEN 'PRESTAMOS'
                ...
                -- Esta condición ya estaba bien para ACT DUDOSOS
                ...
                WHEN dim_1 = 'A_ECORP_Otros_Tarjetas Credito_Gtia Personal' THEN 'A_ECORP_Otros_Tarjetas Credito_Gtia Personal'
                ELSE 'OTROS'
            END AS JER_PRODUCTO,
            -- Forzar A_Balancer a 0
            SUM(CASE WHEN dim_1 = 'A_Balancer' THEN 0
                    WHEN scenario = 'Dwn200' AND NOT is_nb THEN (incexp + diff_incexp_yield) ELSE 0 END) AS Dwn200_stck,
            SUM(CASE WHEN dim_1 = 'A_Balancer' THEN 0
                    WHEN scenario = 'Dwn200' AND is_nb THEN (incexp + diff_incexp_yield) ELSE 0 END) AS Dwn200_Nb,
            SUM(CASE WHEN dim_1 = 'A_Balancer' THEN 0
                    WHEN scenario = 'Base' AND NOT is_nb THEN (incexp + diff_incexp_yield) ELSE 0 END) AS Base_stck,
            SUM(CASE WHEN dim_1 = 'A_Balancer' THEN 0
                    WHEN scenario = 'Base' AND is_nb THEN (incexp + diff_incexp_yield) ELSE 0 END) AS Base_Nb
        FROM pro_pichincha_alquid."result"
        WHERE load_id IN (
            '{load_ids['cierre_base']}',
            '{load_ids['cierre_up']}',
    ...
    ORDER BY CASE JER_PRODUCTO
        ...
        WHEN 'A_ECORP_Otros_Tarjetas Credito_Gtia Personal' THEN 31
        WHEN 'TOTAL GENERAL' THEN 32
        ELSE 999
    END; """
    ...
    Informe_Efecto_Balance= f"""SELECT
                                CASE
                                    WHEN dim_1 IN ('A_Cuentas en otras EECC', 'A_Tesoreria') THEN '1.1 BANCOS'
                                    WHEN dim_1 IN ('A_Ata') THEN '1.2 CARTERA MAYORISTA'
                                    WHEN dim_1 IN ('A_Depositos cedidos') THEN '1.3 DEPOSITOS OTRAS EECC'
                                    ...
                                    ELSE 'OTROS'
                                END AS Jerarquia,
                                SUM(CASE WHEN scenario = 'Base' and load_id = '{load_ids['cierre_base_efecto_balance']}' THEN market_value ELSE 0 END)/1000000 AS Base,
                                SUM(CASE WHEN scenario = 'Up200' and load_id = '{load_ids['cierre_up_efecto_balance']}' THEN market_value ELSE 0 END)/1000000 AS Up200
                                FROM pro_pichincha_alquid.metric
                                GROUP BY 1
                            -- Aquí va el SELECT principal completo con CASE y SUM por escenario
                            UNION ALL
                            SELECT 'TOTAL GRUPO 1' AS Jerarquia,
                                ...
                                SUM(CASE WHEN scenario = 'Up200' and load_id = '{load_ids['cierre_up_efecto_balance']}' THEN market_value ELSE 0 END)/1000000 AS Up200
                            FROM pro_pichincha_alquid.metric
                            WHERE dim_1 in ('A_Cuentas en otras EECC', 'A_Tesoreria', 'A_Ata', 'A_Depositos cedidos',
                            'A_ECORP_Otros_Activo Corto Plazo', 'A_EESP_Otros_Activo Corto Plazo', 'A_POFI_Otros_Ant. Nomina_Gtia Personal',
                                    'A_ECORP_Otros_Cred. Empresa_Gtia Aval', 'A_ECORP_Otros_Cred. Empresa_Gtia Personal',
                                    'A_EESP_Otros_Cred. Empresa_Gtia Aval', 'A_EESP_Otros_Cred. Empresa_Gtia Personal',
                                    'A_SSCC_Otros_Cred. Empresa_Gtia Aval', 'A_SSCC_Otros_Cred. Empresa_Gtia Hipot.', 'A_SSCC_Otros_Cred. Empresa_Gtia Personal',
                                    ...
                                    'A_PIB_Otros_Tarjetas Credito_Gtia Personal', 'A_POFI_Otros_Tarjetas Credito_Gtia Personal',
                                    'A_SSCC_Otros_Tarjetas Credito_Gtia Personal', 'A_SSCCRSG_Otros_Tarjetas Credito_Gtia Personal',
                                    'A_Bonos corporativos', 'A_Bonos soberanos', 'A_Cred._Dudoso', 'A_Efectos_Dudosos', 'A_Prest._Dudoso','FXSWAP_pago', 'FXSWAP_recibo', 'IRS_pago', 'IRS_recibo')
                            UNION ALL
                            SELECT 'TOTAL GRUPO 2' AS Jerarquia,
                                SUM(CASE WHEN scenario = 'Base' and load_id = '{load_ids['cierre_base_efecto_balance']}' THEN market_value ELSE 0 END)/1000000 AS Base,
                                SUM(CASE WHEN scenario = 'Up200' and load_id = '{load_ids['cierre_up_efecto_balance']}' THEN market_value ELSE 0 END)/1000000 AS Up200
                            FROM pro_pichincha_alquid.metric
                            where dim_1 in ('P_Cuentas de otras EECC', 'P_ECORP_P. Plazo', 'P_PIB_P. Plazo', 'P_POFI_P. Plazo', 'P_SSCC_P. Plazo',  'P_ECORP_Ctas No Remuneradas', 'P_EESP_Ctas No Remuneradas', 'P_PIB_Ctas No Remuneradas',
                                    'P_POFI_Ctas No Remuneradas', 'P_SSCC_Ctas No Remuneradas', 'P_SSCCFRO_Ctas No Remuneradas',
                                    'P_PIB_Ctas Remuneradas', 'P_POFI_Ctas Remuneradas', 'P_SSCC_Ctas Remuneradas', 'P_Ata', 'P_Emisiones T2')
                            UNION ALL
                            SELECT 'TOTAL RENTA FIJA' AS Jerarquia,
                                SUM(CASE WHEN scenario = 'Base' and load_id = '{load_ids['cierre_base_efecto_balance']}' THEN market_value ELSE 0 END)/1000000 AS Base,
                                SUM(CASE WHEN scenario = 'Up200' and load_id = '{load_ids['cierre_up_efecto_balance']}' THEN market_value ELSE 0 END)/1000000 AS Up200
                            FROM pro_pichincha_alquid.metric
                            where DIM_1 in ('A_Bonos corporativos', 'A_Bonos soberanos','IRS_pago','IRS_recibo', 'FXSWAP_pago', 'FXSWAP_recibo' )
                            ORDER BY CASE Jerarquia
    ...
    Informe_Efecto_Curva = f"""SELECT
                                ...
                                ELSE 'OTROS'
                                END AS Jerarquia,
                                SUM(CASE WHEN scenario = 'Base' and load_id = '{load_ids['cierre_base_efecto_curva']}' THEN market_value ELSE 0 END)/1000000 AS Base,
                                SUM(CASE WHEN scenario = 'Up200' and load_id = '{load_ids['cierre_up_efecto_curva']}' THEN market_value ELSE 0 END)/1000000 AS Up200
                            FROM pro_pichincha_alquid.metric
                            GROUP BY 1
                            -- Aquí va el SELECT principal completo con CASE y SUM por escenario
                            UNION ALL
    ...
    Informe_Efecto_Curva_Mes_Actual = f"""SELECT
    CASE
        WHEN dim_1 IN ('A_Cuentas en otras EECC', 'A_Tesoreria') THEN '1.1 BANCOS'
        WHEN dim_1 IN ('A_Ata') THEN '1.2 CARTERA MAYORISTA'
        WHEN dim_1 IN ('A_Depositos cedidos') THEN '1.3 DEPOSITOS OTRAS EECC'
    ...
    Informe_Datos_Medios_01= f"""select
    CASE
    ...
    when '1.2 CARTERA MAYORISTA.F' THEN 9
    WHEN '1.3 DEPOSITOS OTRAS EECC.V' THEN 10
    WHEN '1.4 INVERSION CREDITICIA.V' THEN 11
    WHEN '1.5 RENTA FIJA.V' THEN 12
    WHEN '1.6 ACTIVOS DUDOSOS.V' THEN 13
    WHEN '1.7 ACTIVOS NO SENSIBLE.V' THEN 14
    WHEN '2.1 DEPOSITOS OTRAS EECC.F' THEN 15
    WHEN '2.2 DEPOSITOS A PLAZO DE CLIENTES.F' THEN 16
    WHEN '2.3 DEPOSITOS A LA VISTA DE CLIENTES.F' THEN 17
    WHEN '2.4 PASIVOS NO SENSIBLES.F' THEN 18
    WHEN '2.5 CARTERA MAYORISTA.F' THEN 19
    WHEN '2.1 DEPOSITOS OTRAS EECC.V' THEN 20
    WHEN '2.2 DEPOSITOS A PLAZO DE CLIENTES.V' THEN 21
    WHEN '2.4 PASIVOS NO SENSIBLES.V' THEN 22
    WHEN '2.5 CARTERA MAYORISTA.V' THEN 23
    ELSE 999
    END;
    """
    ...
    SELECT
        ...
        WHEN dim_1 IN (
            ...
            'A_SSCC_Otros_Prest. Empresas_Gtia Personal', 'A_SSCC_Otros_Prest. Hipotecas_Gtia Hipot.',
            'A_PIB_Otros_Tarjetas Credito_Gtia Personal', 'A_POFI_Otros_Tarjetas Credito_Gtia Personal',
            'A_SSCC_Otros_Tarjetas Credito_Gtia Personal', 'A_SSCCRSG_Otros_Tarjetas Credito_Gtto Personal'
        ) AND NOT is_floating THEN '1.4 INVERSION CREDITICIA.F'
        WHEN dim_1 IN ('A_Bonos corporativos', 'A_Bonos soberanos') AND NOT is_floating THEN '1.5 RENTA FIJA.F'
        WHEN dim_1 IN ('A_Cred._Dudoso', 'A_Efectos_Dudosos', 'A_Prest._Dudoso') AND NOT is_floating THEN '1.6 ACTIVOS DUDOSOS.F'
        WHEN dim_1 IN ('A_Resto no sensible', 'A_Tesoreria_Admin.') AND NOT is_floating THEN '1.7 ACTIVOS NO SENSIBLE.F'
        ...
        WHEN dim_1 IN (
            ...
            'A_ECORP_Otros_Prest. Empresas_Gtia Aval', 'A_ECORP_Otros_Prest. Empresas_Gtia Hipot.', 'A_ECORP_Otros_Prest. Empresas_Gtia Personal',
            'A_ECORP_Otros_Prest. Empresas_Gtia Prenda', 'A_ECORP_Otros_Prest. Hipotecas_Gtia Hipot.', 'A_ECORP_Otros_Prest. Hipotecas_Gtia Personal',
            'A_EESP_Otros_Prest. Empresas_Gtia Aval', 'A_EESP_Otros_Prest. Empresas_Empresas_Gtia Personal', 'A_EESP_Otros_Prest. Hipotecas_Gtia Hipot.',
            'A_EESP_Otros_Prest. Promotor_Gtia Hipot.', 'A_PIB_Otros_Prest. Consumo_Gtia Personal', 'A_PIB_Otros_Prest. Hipotecas_Gtia Hipot.',
            'A_PIB_Otros_Prest. Hipotecas_Gtia Personal', 'A_POFI_Otros_Prest. Consumo_Gtia Personal', 'A_POFI_Otros_Prest. Consumo_Gtia Prenda',
            'A_POFI_Otros_Prest. Empresas_Gtia Aval', 'A_POFI_Otros_Prest. Empresas_Gtia Hipot.', 'A_POFI_Otros_Prest. Empresas_Gtia Personal',
            'A_POFI_Otros_Prest. Hipotecas_Gtia Hipot.', 'A_POFI_Otros_Prest. Hipotecas_Gtia Personal',
        ...
        SUM((date_diff('day', Reprdate, matdate) * Notional_today)) / NULLIF(SUM(Notional_today), 0) / 30 AS reprecio_medio,
    ...
    Informe_Datos_Medios_03_01= f"""select
    case
        when dim_1 in ('P_ECORP_Ctas No Remuneradas', 'P_EESP_Ctas No Remuneradas', 'P_PIB_Ctas No Remuneradas',
        'P_POFI_Ctas No Remuneradas', 'P_SSCC_Ctas No Remuneradas', 'P_SSCCFRO_Ctas No Remuneradas') and nmd_ref = 'NMD_MAY_NOFIN' then 'CTAS NO REMUNERADAS_1'     
        when dim_1 in ('P_ECORP_Ctas No Remuneradas', 'P_EESP_Ctas No Remuneradas', 'P_PIB_Ctas No Remuneradas',
        'P_POFI_Ctas No Remuneradas', 'P_SSCC_Ctas No Remuneradas', 'P_SSCCFRO_Ctas No Remuneradas') and nmd_ref = 'NMD_MIN_NONTR' then 'CTAS NO REMUNERADAS_3'     
        when dim_1 in ('P_ECORP_Ctas No Remuneradas', 'P_EESP_Ctas No Remuneradas', 'P_PIB_Ctas No Remuneradas',
        'P_POFI_Ctas No Remuneradas', 'P_SSCC_Ctas No Remuneradas', 'P_SSCCFRO_Ctas No Remuneradas') and nmd_ref = 'NMD_MIN_TR' then 'CTAS NO REMUNERADAS_4'   
    when dim_1 in ('P_PIB_Ctas Remuneradas', 'P_POFI_Ctas Remuneradas', 'P_SSCC_Ctas Remuneradas') and nmd_ref = 'NMD_MAY_NOFIN' then 'CTAS REMUNERADAS_1'
    when dim_1 in ('P_PIB_Ctas Remuneradas', 'P_POFI_Ctas Remuneradas', 'P_SSCC_Ctas Remuneradas') and nmd_ref = 'NMD_MAY_FIN' then 'CTAS REMUNERADAS_2'
    when dim_1 in ('P_PIB_Ctas Remuneradas', 'P_POFI_Ctas Remuneradas', 'P_SSCC_Ctas Remuneradas') and nmd_ref = 'NMD_MIN_NONTR' then 'CTAS REMUNERADAS_3'
    when dim_1 in ('P_PIB_Ctas Remuneradas', 'P_POFI_Ctas Remuneradas', 'P_SSCC_Ctas Remuneradas') and nmd_ref = 'NMD_MIN_TR' then 'CTAS REMUNERADAS_4'      
    else 'Otros'
    end as jerarquia,
        nmd_ref,
        SUM(notional_today) as notional,
    ...
    when 'CTAS REMUNERADAS_1' then 4
    when 'CTAS REMUNERADAS_3' then 6
    when 'CTAS REMUNERADAS_4' then 7
        when 'CTAS REMUNERADAS_2' then 8
    else 999
    end; """
    ...
    # Filas a evitar
    filas_a_saltar_Datos_Medios_01   = {14, 18, 19, 20}
